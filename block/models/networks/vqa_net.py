...
def mask_softmax(x, lengths):#, dim=1)
    ...
    # torch.isnan(x3_sum).float().sum().item()
    #x4.data[torch.isnan(x4.data)] = 0
    if torch.isnan(x2).float().sum().item() > 0:
        import ipdb;ipdb.set_trace()

    if torch.isnan(x4).float().sum().item() > 0:
        import ipdb;ipdb.set_trace()
    return x4
...
class VQANet(nn.Module):
    ...
    def process_answers(self, out):
        ...
        pred.squeeze_()
        out['answers'] = [self.aid_to_ans[pred[i].item()] for i in range(batch_size)]
        out['answer_ids'] = [pred[i].item() for i in range(batch_size)]
        return out
