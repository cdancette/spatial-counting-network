...
class Engine(object):
    ...
    def load_state_dict(self, state):
        ...
        self.iteration = state.get("iteration", None)
    ...
    def eval(self):
        ...
        if Options()["dataset"]["eval_split"]:
            ...
            self.eval_epoch(
                self.model, self.dataset["eval"], self.epoch - 1, logs_json=True
            )
    ...
    def train(self):
        ...
        while self.epoch < Options()["engine"]["nb_epochs"]:
            self.train_epoch(
                self.model, self.dataset["train"], self.optimizer, self.epoch
            )
            ...
            if Options()["dataset"]["eval_split"]:
                ...
                if (
                    "saving_criteria" in Options()["engine"]
                    and Options()["engine"]["saving_criteria"] is not None
                ):
                    for saving_criteria in Options()["engine"]["saving_criteria"]:
                        if self.is_best(out, saving_criteria):
                            name = saving_criteria.split(":")[0]
                            Logger()(
                                "Saving best checkpoint for strategy {}".format(name)
                            )
                            self.save(
                                Options()["exp"]["dir"],
                                "best_{}".format(name),
    ...
    def train_epoch(self, model, dataset, optimizer, epoch, mode="train"):
        ...
        for i, batch in enumerate(batch_loader):
            ...
            do_backward = Options()["engine"].get("do_backward", True)
            if do_backward:
            ...
            Logger().log_value(
                f"{mode}_batch.timer.process", timer["process"], should_print=False
            )
            Logger().log_value(
                f"{mode}_batch.timer.load", timer["load"], should_print=False
            )
            ...
            if i % Options()["engine"]["print_freq"] == 0:
                Logger()(
                    "{}: epoch {} | batch {}/{}".format(
                        mode, epoch, i, len(batch_loader) - 1
                    )
                )
                Logger()(
                    "{} elapsed: {} | left: {}".format(
                ...
                Logger()(
                    "{} process: {:.5f} | load: {:.5f}".format(
                        " " * len(mode), timer["process"], timer["load"]
                    )
                )
                Logger()(
                    "{} loss: {:.5f}".format(" " * len(mode), out["loss"].data.item())
                )
        ...
        for key, value in out_epoch.items():
            try:
                Logger().log_value(
                    f"{mode}_epoch." + key, sum(value) / len(value), should_print=True
                )
            except:
                Logger()(
                    f"Error with key {key} and value {value}", log_level=Logger.ERROR
                )
                traceback.print_exc()
                breakpoint()
    ...
    def eval_epoch(self, model, dataset, epoch, mode="eval", logs_json=True):
        ...
        for i, batch in enumerate(batch_loader):
            ...
            Logger().log_value(
                "{}_batch.timer.process".format(mode),
                timer["process"],
                should_print=False,
            )
            Logger().log_value(
                "{}_batch.timer.load".format(mode), timer["load"], should_print=False
            )
            ...
            for key, value in out.items():
                ...
                out_epoch[key].append(value)
                Logger().log_value(
                    "{}_batch.{}".format(mode, key), value, should_print=False
                )
        ...
        for key, value in out.items():
            Logger().log_value(
                "{}_epoch.{}".format(mode, key), value, should_print=True
            )
    ...
    def is_best(self, out, saving_criteria):
        ...
        if name not in out:
            raise KeyError(
                "'--engine.saving_criteria' named '{}' not in outputs '{}'".format(
                    name, list(out.keys())
                )
            )
    ...
    def load(self, dir_logs, name, model, optimizer, map_location=None):
        ...
        path_engine = path_template.format(name, "engine")

        if not os.path.isfile(path_model):
            if Options()["exp"].get("resume_or_start", False):
                Logger()(
                    "No checkpoint. Starting from scratch", log_level=Logger.WARNING
                )
                return False
            else:
                raise RuntimeError(
                    f"No checkpoint at {path_model}"
                )

        Logger()(f"Loading model {path_model}")
        model_state = torch.load(path_model, map_location=map_location)
        model.load_state_dict(model_state)

        if Options()["dataset"]["train_split"] is not None:
            if not os.path.isfile(path_optimizer):
                Logger()("No optimizer checkpoint", log_level=Logger.WARNING)
                return False

        if not os.path.isfile(path_engine):
        ...
        if Options()["dataset"]["train_split"] is not None:
            ...
            optimizer_state = torch.load(
                path_optimizer, map_location=map_location
            )
