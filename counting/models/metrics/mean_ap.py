...
"""
    ...
    This is taken from  https://github.com/MathGaron/mean_average_precision/tree/master/mean_average_precision
"""
...
class DetectionMAP:
    ...
    def evaluate(self, pred_bb, pred_classes, pred_conf, gt_bb, gt_classes):
        ...
        for accumulators, r in zip(self.total_accumulators, self.pr_scale):
            ...
            self.evaluate_(
                IoUmask, accumulators, pred_classes, pred_conf, gt_classes, r
            )

    @staticmethod
    def evaluate_(
        IoUmask, accumulators, pred_classes, pred_conf, gt_classes, confidence_threshold
    ):
    ...
    def get_scores(self, interpolated=True):
    ...
    def plot(self, interpolated=True, class_names=None):
        ...
        plt.suptitle(
            "Mean average precision : {:0.2f}".format(
                sum(mean_average_precision) / len(mean_average_precision)
            )
        )
        fig.tight_layout()
